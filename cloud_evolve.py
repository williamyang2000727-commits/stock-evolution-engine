...
def run_single_test(args):
    ...
    score = (
        avg_ret * 1.5 +                  # 平均報酬 (最重要！寧缺勿濫，每筆都要賺)
        win_rate * 0.4 +                 # 勝率 (穩定性核心)
        total_ret * 0.02 +               # 總報酬 (大幅降權，避免過度擬合特定爆發股)
        min(pf, 10) * 0.5 +              # 利潤因子 (風險報酬比，上限拉高)
        consistency * 5.0 +              # 前後段穩定性
        n_trades * 0.05 +                # 交易筆數 (微幅獎勵，不鼓勵亂買)
        -wasted * 0.1                    # 白做工懲罰 (無效交易扣分)
    )
