"""
動態股票池回測 v2 — D+1開盤賣 + D+1收盤買 + 交易成本 + 同天買賣 + 換股
"""
import numpy as np
import json, os, sys, pickle, types
...
all_tickers = [t for t, df in data.items() if len(df) >= 400]
min_len = min(len(data[t]) for t in all_tickers)
print(f"股票：{len(all_tickers)} 檔 | 天數：{min_len}")

n_stocks = len(all_tickers)
close = np.zeros((n_stocks, min_len), dtype=np.float32)
high = np.zeros((n_stocks, min_len), dtype=np.float32)
low = np.zeros((n_stocks, min_len), dtype=np.float32)
opn = np.zeros((n_stocks, min_len), dtype=np.float32)
volume = np.zeros((n_stocks, min_len), dtype=np.float32)
dates = None
for si, t in enumerate(all_tickers):
    h = data[t]
    close[si] = h["Close"].values[-min_len:].astype(np.float32)
    high[si] = h["High"].values[-min_len:].astype(np.float32)
    low[si] = h["Low"].values[-min_len:].astype(np.float32)
    opn[si] = h["Open"].values[-min_len:].astype(np.float32)
    volume[si] = h["Volume"].values[-min_len:].astype(np.float32)
    if dates is None: dates = h.index[-min_len:]
...
loss_a = np.where(delta < 0, -delta, 0)
ag = np.zeros_like(close); al = np.zeros_like(close)
for i in range(14, nd):
    if i == 14: ag[:, i] = np.mean(gain[:, :14], axis=1); al[:, i] = np.mean(loss_a[:, :14], axis=1)
    else: ag[:, i] = (ag[:, i-1]*13 + gain[:, i-1])/14; al[:, i] = (al[:, i-1]*13 + loss_a[:, i-1])/14
...
for day in range(60, nd - 1):
    ...
    # Phase 1: 賣出（D+1 開盤價，檢查所有 3 個 slot）
    for h in range(3):
        ...
        if sell and day+1<nd:
            ...
            trades.append({"ticker":all_tickers[si],"name":get_name(all_tickers[si]),
...
print(f"\n{'='*60}")
print(f" 動態回測 v2（D+1開盤賣 + D+1收盤買 + 扣交易成本）")
