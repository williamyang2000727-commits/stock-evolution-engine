...
def compute_indicators(margin_df: pd.DataFrame, cache_df: pd.DataFrame, reference_dates=None) -> np.ndarray:
    ...
    # BUG #14/#15/#16 修正（2026-04-25）：「沒資料」情境統一填 -999（sentinel），讓 V34 kernel/cpu_replay 用 > -900 guard 擋掉
    ...
    heat = heat.fillna(-999.0)  # 只把 NaN 填 -1，真正的 heat=0 保持 0
    ...
    accel = accel.fillna(-999.0)  # NaN = 5 天前沒融資 = 無效
    ...
    short_ratio = short_ratio.fillna(-999.0)
    ...
    # 如果 accel 是 sentinel（無效），diverge 也應該是 sentinel（依賴 accel）
    _accel_valid_mask = accel.values > -900.0
    diverge_vals = (accel.values - price5).clip(-200, 200)
    diverge_vals[~_accel_valid_mask] = -999.0
...
def main():
    ...
    print(f"{OUT_META}")
    print(f"\n=== 指標統計（sentinel = -999，有效值 > -900）===")
    names = ["margin_heat", "margin_accel", "short_ratio", "offset_rate", "margin_diverge"]
    for i, name in enumerate(names):
        s = tensor[:, :, i]
        valid = s[s > -900.0]
        sentinel_pct = (s <= -900.0).sum() / s.size * 100 if s.size else 0
        if len(valid) > 0:
            print(f"  {name:<17s} valid_mean={valid.mean():7.2f} std={valid.std():7.2f} "
                  f"min={valid.min():7.2f} max={valid.max():7.2f}  sentinel={sentinel_pct:.1f}%")
        else:
